...
const FeatureControls = ({ featuresGeoJSON, onFeaturesChange, onAddFeatureClick }) => {
    ...
    const handleClearAllFeatures = () => {
        ...
        if (
            window.confirm(
                `Are you sure you want to remove all ${featuresGeoJSON.features.length} features? This action cannot be undone.`
            )
        ) {
                    ...
                    {featuresGeoJSON.features.length > 0 && (
                        <div className="feature-summary">
                            <div className="feature-type-counts">
                                <div>
                                    🪸 Coral Tables:{' '}
                                    {featuresGeoJSON.features.filter((f) => f.properties.type === 'coral_table').length}
                                </div>
                                <div>
                                    🐠 Natural:{' '}
                                    {
                                        featuresGeoJSON.features.filter((f) => f.properties.type === 'natural_feature')
                                            .length
                                    }
                                </div>
                                <div>
                                    📊 Monitoring:{' '}
                                    {
                                        featuresGeoJSON.features.filter((f) => f.properties.type === 'monitoring_point')
                                            .length
                                    }
                                </div>
                                <div>
                                    📍 Other:{' '}
                                    {featuresGeoJSON.features.filter((f) => f.properties.type === 'other').length}
                                </div>
                            </div>
                        </div>
                    )}
